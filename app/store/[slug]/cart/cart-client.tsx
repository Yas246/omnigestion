...
/**
 * Cart checkout — CLIENT part. Receives the storefront `config` from the server
 * page (fetched server-side in page.tsx) so the theme/colors/fonts are applied
...
export function CartClient({ slug, config, companyName }: { slug: string; config: StorefrontConfig | null; companyName: string | null }) {
  ...
  // Auto-close auth modal when buyer logs in.
  useEffect(() => {
