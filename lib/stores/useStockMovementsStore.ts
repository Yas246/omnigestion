...
interface StockMovementsState {
  ...
  fetchMovements: (options?: { reset?: boolean; pageSize?: number }) => Promise<void>;
...
export const useStockMovementsStore = create<StockMovementsState>()(
  subscribeWithSelector((set, get) => ({
    ...
    loadMore: async () => {
      ...
      if (!hasMore || loading) {
        console.log('[loadMore] Pas plus de mouvements ou chargement en cours');
        return;
      }
    ...
    setProductFilter: (productId) => {
      console.log('[setProductFilter] Changement filtre produit', { productId });
      set((state) => ({
 ...
 */
export const selectMovements = () => useStockMovementsStore.getState().getFilteredMovements();
