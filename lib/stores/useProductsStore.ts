...
interface ProductsState {
  ...
  optimisticUpdateProduct: (productId: string, updates: Partial<Product>) => void;
...
export const useProductsStore = create<ProductsState>()(
  subscribeWithSelector((set, get) => ({
    ...
    fetchProducts: async (companyId, options = {}) => {
      ...
      try {
        const { data: newProducts, hasMore, lastDoc: newLastDoc } =
    ...
    setWarehouseFilter: (warehouseId) => {
      console.log('[setWarehouseFilter] Changement filtre entrepôt', { warehouseId });
      set((state) => ({
    ...
    setCategoryFilter: (category) => {
      console.log('[setCategoryFilter] Changement filtre catégorie', { category });
      set((state) => ({
    ...
    setPriceRangeFilter: (min, max) => {
      console.log('[setPriceRangeFilter] Changement filtre prix', { min, max });
      set((state) => ({
    ...
    optimisticCreateProduct: (product) => {
      console.log('[optimisticCreateProduct] Ajout produit optimiste', { productId: product.id });
      set((state) => ({
    ...
    optimisticUpdateProduct: (productId, updates) => {
      console.log('[optimisticUpdateProduct] Mise à jour optimiste', { productId, updates });
      set((state) => ({
    ...
    optimisticDeleteProduct: (productId) => {
      console.log('[optimisticDeleteProduct] Suppression optimiste', { productId });
      set((state) => ({
    ...
    syncProduct: async (productId) => {
      ...
      try {
        ...
        if (product) {
          ...
          console.log('[syncProduct] Produit retiré (supprimé)', { productId });
        }
      } catch (error) {
    ...
    syncProductsStockLocations: async (companyId, productIds) => {
      ...
      console.log('[syncProductsStockLocations] Synchronisation stock_locations', {
        count: productIds.length,
      });

      try {
        const locationsMap = await productsApi.fetchProductsStockLocations(companyId, productIds);
    ...
    getFilteredProducts: () => {
      ...
      if (filters.warehouseId) {
        filtered = filtered.filter((p) => {
          ...
          return locations.some((l) => l.warehouseId === filters.warehouseId && l.quantity > 0);
        });
      }
 ...
 */
export const selectProducts = () => useProductsStore.getState().getFilteredProducts();
