  ...
  persist(
    subscribeWithSelector((set, get) => ({
      ...
      setCreditRangeFilter: (min, max) => {
        console.log("[setCreditRangeFilter] Changement filtre crédit", {
          min,
          max,
        });
        set((state) => ({
      ...
      optimisticCreateClient: (client) => {
        console.log("[optimisticCreateClient] Ajout client optimiste", {
          clientId: client.id,
        });
        set((state) => ({
      ...
      optimisticUpdateClient: (clientId, updates) => {
        console.log("[optimisticUpdateClient] Mise à jour optimiste", {
          clientId,
          updates,
        });
        set((state) => ({
      ...
      optimisticDeleteClient: (clientId) => {
        console.log("[optimisticDeleteClient] Suppression optimiste", {
          clientId,
        });
        set((state) => ({
      ...
      getFilteredClients: () => {
        ...
        if (filters.minCredit !== null) {
          filtered = filtered.filter(
            (c) => c.currentCredit >= filters.minCredit!,
          );
        }
        if (filters.maxCredit !== null) {
          filtered = filtered.filter(
            (c) => c.currentCredit <= filters.maxCredit!,
          );
        }
...
export const selectClients = () =>
  useClientsStore.getState().getFilteredClients();
...
export const useClientsLoading = () =>
  useClientsStore((state) => state.loading);
export const useClientsError = () => useClientsStore((state) => state.error);
export const useClientsHasMore = () =>
  useClientsStore((state) => state.hasMore);
export const useClientsFilters = () =>
  useClientsStore((state) => state.filters);
