...
/**
 * Hook pour les mouvements de stock avec écoute temps réel GLOBAL
 *
 ...
 */
export function useStockMovementsRealtime(options?: { limit?: number }) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const [limit, setLimit] = useState(options?.limit || 20);

  const { data: movements = [], isLoading, error } = useQuery<any[]>({
    queryKey: ['companies', user?.currentCompanyId, 'stockMovements'],
    queryFn: async () => [],
  ...
  useEffect(() => {
    if (user?.currentCompanyId) {
      realtimeService.startStockMovementsListener(queryClient, user.currentCompanyId, limit);
    }
