...
/**
 * Hook pour les caisses avec écoute temps réel GLOBAL
 *
...
export function useCashRegistersRealtime() {
  ...
  const { data: cashRegisters = [], isLoading, error } = useQuery<any[]>({
    queryKey: ['companies', user?.currentCompanyId, 'cashRegisters'],
    queryFn: async () => [],
  ...
  useEffect(() => {
    if (user?.currentCompanyId) {
      realtimeService.startCashRegistersListener(queryClient, user.currentCompanyId);
    }
